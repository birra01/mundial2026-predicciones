...
HOME_ADV = 0.10   # +10% prob victoria local
ELO_K = 32
ELO_HOME = 60
...
class WorldCupEngine:
    ...
    def predict_match(self, home_team, away_team):
        ...
        # ===== 1. MODELO ESTADÍSTICO AJUSTADO POR RIVAL =====
        stat_contributions = []
        for key, weight in WEIGHTS.items():
            hv = ht['avg_stats'].get(key, 1.0)
            av = at['avg_stats'].get(key, 1.0)

            # ratio > 1 = mejor que la media contra este tipo de rival
            # ratio < 1 = peor que la media
            if weight < 0:
                # Stats defensivas: invertir
        ...
        # Convertir a probabilidad (tanh suave, centrado)
        stat_prob_home = (math.tanh(stat_score * 0.8) + 1) / 2

        # ===== 2. ELO =====
        elo_diff = (self.elo[home_team] + ELO_HOME) - self.elo[away_team]
        elo_prob_home = 1 / (1 + 10 ** (-elo_diff / 400))

        # ===== 3. POISSON CON GOLES REALES =====
        # Usar promedios crudos cruzados (ataque vs defensa)
        h_gf = ht['avg_goals_for']
        h_ga = ht['avg_goals_against']
        a_gf = at['avg_goals_for']
        a_ga = at['avg_goals_against']
        
        # Goles esperados: media del ataque propio y defensa rival
        exp_hg = (h_gf + a_ga) / 2
        exp_ag = (a_gf + h_ga) / 2

        # Poisson truncado (0..7 goles, suficiente)
        prob_hw = prob_d = prob_aw = 0
        for hg in range(8):
            for ag in range(8):
                p = (math.exp(-exp_hg) * exp_hg**hg / math.factorial(hg) *
        ...
        form_home = h_form / total_form if total_form > 0 else 0.5

        # ===== 5. MEZCLA FINAL (más conservadora) =====
        # Elo: 35%, Stats ajustadas: 30%, Poisson: 25%, Forma: 10%
        comp_h = elo_prob_home * 0.35 + stat_prob_home * 0.30 + prob_hw * 0.25 + form_home * 0.10
        comp_a = (1 - elo_prob_home) * 0.35 + (1 - stat_prob_home) * 0.30 + prob_aw * 0.25 + (1 - form_home) * 0.10
        comp_d = prob_d * 0.25 + max(0, 0.20 - abs(stat_prob_home - 0.5)) * 0.15

        total = comp_h + comp_d + comp_a
        if total > 0:
            comp_h /= total
            comp_d /= total
            comp_a /= total
        ...
        return {
            ...
            'narrative': narr,
            'model_breakdown': {
                'elo': round(elo_prob_home * 100, 1),
                'statistical': round(stat_prob_home * 100, 1),
                'poisson': round(prob_hw * 100, 1),
            },
        }
